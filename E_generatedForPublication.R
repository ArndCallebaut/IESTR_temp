...
esps[,1]=runif(nb_species )*marge + (Tmin_alt_0 + Tadd_alt_1)
esps[,2]=runif(nb_species )*marge*0.1
esps[,3]=runif(nb_species )*marge*0.1+esps[,2]
esps[,4]=runif(nb_species )*0.1
esps[,5]= sample(1:3, nb_species  ,replace=TRUE)
...
#QCQCQCTM
#ccm = ccmm1 = function(map,Tgain,T)
...
for (i in 1:nb_species){
  ...
  suitnow = suities[[i]][[1]]
  maty <- matrix(runif( ncol*nrow),ncol = ncol)                # Specify number of columns
  maty <- (maty) * seq(0,1,length=nrow)^2
  
  suitnow>0.9
  pres = (suitnow>0.9) * (maty>0.3)
...
cc0 = c_maps[[1]]
image.nan.better(cc0, col =hea,zlim=range(-10,20),outside.below.color='red',outside.above.color='brown',na.color='navy')
...
ccN = c_maps[[PERIOD]]
image.nan.better(ccN, col =hea,zlim=range(-10,20),outside.below.color='red',outside.above.color='brown',na.color='navy')
...
HM2 = map*0
HM2[which(resu0>=0,arr.ind=TRUE)] = valN
