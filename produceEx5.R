...
ggplot(data, aes(lontitude, latitude, fill= cost)) + geom_tile() + scale_fill_gradient(low="blue", high="red",limits=c(100,150)) + theme_ipsum()
...
for (i in 1:length(indices_pres)){
  for (j in 1:length(cm[[1]][,1])){
    rvb_tensor3[possibilities[j,1],possibilities[j,2],1]=cm[[i]][j,indices_pres[i]]
...
#####
##### Afficher les endroits où on plante V2
#####
rvb_tensor3 = array(0.5,c(nrow,ncol,3))
...
plotRGB(flip(raster_RGB))
legend("topright",c("Surviving sites", "Naturally colonised sites", "Not surviving sites"), cex=1.0, bty="y",
       fill=c("green","yellow","red"),inset=.04)
