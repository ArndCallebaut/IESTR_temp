...
plot(NULL)
legend("topright",c("Area occupied by species","Area becomming unsuitable", "Area always suitable", "Area becomming suitable"), cex=3.0, bty="y",
...
resultat = rcpp_algorithm_opt(ph,vt,po,cost1,pres[nrow:1,],cm,gss,vv,threshold,confidence,npop,nsur,ngen,ntp)
...
cost = (!is.na(map)) * cost_map(nrow,ncol,cost_mapping)
#cost = cost_map(nrow,ncol,cost_mapping)
ggplot(data, aes(lontitude, latitude, fill= cost)) + geom_tile() + scale_fill_gradient(low="blue", high="red",limits=c(100,500)) + theme_ipsum()
if(class(cost)=="dtCMatrix"){
  cost = as(as(cost,"dgCMatrix"),"dgCMatrix")
}else{
  cost = as(cost,"dgCMatrix")
...
data <- expand.grid(X=nrow, Y=ncol)
colnames(data) = c("lontitude","latitude")
...
cost = (!is.na(map)) * cost_map(nrow,ncol,cost_mapping)
ggplot(data, aes(lontitude, latitude, fill= cost)) + geom_tile() + scale_fill_gradient(low="blue", high="red",limits=c(100,100)) + theme_ipsum()
if(class(cost)=="dtCMatrix"){
  cost = as(as(cost,"dgCMatrix"),"dgCMatrix")
}else{
  cost = as(cost,"dgCMatrix")
...
legend("topright",legend = c("Surviving sites", "Naturally colonised sites", "Not surviving sites", "Planting sites AM", "Colonised sites due to AM"), cex=3.0, bty="y",
       fill=c("green","yellow","red","blue","purple"), inset = 0)
